...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/anjing-liu/3x-ui-1/main/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
custom_version() {
    ...
    download_link="https://raw.githubusercontent.com/anjing-liu/3x-ui-1/master/install.sh"
...
uninstall() {
    ...
    echo -e "${green}bash <(curl -Ls https://raw.githubusercontent.com/anjing-liu/3x-ui-1/master/install.sh)${plain}"
    echo ""
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/anjing-liu/3x-ui-1/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
——————————————————————
  ...
  ${yellow}https://github.com/anjing-liu/3x-ui-1${plain}
