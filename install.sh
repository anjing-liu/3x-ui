...
config_ssl_domain() {
    ...
    echo ""
    echo -e "${red}注意：配置域名访问需要：${plain}"
    echo -e "${yellow}  1. 已有解析到本服务器的域名${plain}"
    echo -e "${yellow}  2. 域名已托管到 Cloudflare（或其他DNS）${plain}"
    echo -e "${yellow}  3. 80 和 443 端口已放行${plain}"
    echo ""
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        echo ""
        sleep 2
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(arch).tar.gz https://github.com/anjing-liu/3x-ui/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/anjing-liu/3x-ui/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz"
        echo ""
        echo -e "--------------------------------------------"
...
echo ""
sleep 2
echo -e "${green}VPS评测网站：https://www.1373737.xyz/${plain}"
echo -e "${yellow}服务器购买地址：https://my.frantech.ca/aff.php?aff=4337${plain}"
echo -e "${yellow}YouTube频道：https://www.youtube.com/@cyndiboy7881${plain}"
echo ""
