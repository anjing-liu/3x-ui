...
echo ""
last_version=$(curl -Ls "https://api.github.com/repos/anjing-liu/3x-ui-1/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
# 获取 x-ui 版本
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        echo ""
        sleep 2
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-$(arch).tar.gz https://github.com/anjing-liu/3x-ui-1/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/anjing-liu/3x-ui-1/releases/download/${last_version}/x-ui-linux-$(arch).tar.gz"
        echo ""
        echo -e "--------------------------------------------"
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/anjing-liu/3x-ui-1/main/x-ui.sh
